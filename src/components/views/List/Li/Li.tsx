...
const Li = ({ slug, gist, collection }: Props) => {
  ...
  const title = hasTitle
    ? hasTitle.name.slice(0, -10).replace(/^[_]+/, '')
    : gist.files[0].name;

  const lang = [
    ...new Set(
      gist.files.map(item => item.extension && item.extension.replace('.', ''))
    ),
  ];
    ...
    <Container>
      <li>
        <NavLink
          ...
          onDragEnd={() =>
            dispatch(setDragged({ name: '', currentCollection: '' }))
          }
        >
          ...
          {lang &&
            lang.map((extension, i) => ( extension &&
